...
Next.js 14 + Supabase 기반의 연맹 SVS(관직) 예약·배정 시스템입니다.
...
| `GOOGLE_FORM_WEBHOOK_SECRET` | 구글 폼 웹훅 인증 비밀값 (`X-Webhook-Secret`과 동일) |
| `NEXT_PUBLIC_BASE_URL` | (선택) Admin 시크릿 URL 표시용 공개 origin |
...
> **Delete vs Cancel:** Delete는 **배정 전**(`last_assignment_run` 없음) Search에서만 표시. Cancel은 **배정 후** Schedule Grid에서 슬롯 단위 취소 + 대기자 승격. 둘 다 `audit_log`에 스냅샷을 남깁니다.

#### 플레이어 신청

| # | 상황 | 조건 | 결과 | 사용자 메시지 |
|---|------|------|------|---------------|
| 1 | 정상 첫 신청 | 구글 폼 또는 시크릿 URL (**Open secret URL**) | `submit_multi_day_reservation` RPC → `players` upsert + `preferences` DELETE(해당 player+cycle) + INSERT | *Your application has been received.* |
| 2 | 같은 `player_id` 재제출 | 동일 사이클에 기존 `preferences` 있음 | 동일 RPC, 전체 교체 (+ `audit_log` `resubmit_preference` 스냅샷) | *Your application has been updated.* |
...
| 7. 사후 조정 | 있음 | Grid Cancel, Waitlist, **Pending** | 취소·승격·배정 후 재신청 확인 |
...
| `/admin` | 로그인 후 | Secret URL 표시·**Open/Close secret URL**·배정·검색·그리드·Pending·Reset (구글 폼 마감은 Google Forms에서 별도) |
| `/admin/guide` | 로그인 후 | How to use (Admin / Player / **스택** 탭) |
...
| 메서드 | 경로 | 인증 | 설명 |
|--------|------|------|------|
| POST | `/api/google-form-submit` | `X-Webhook-Secret` 헤더 | 구글 폼 payload → `submitMultiDayReservationRpc` (시크릿 URL과 동일 RPC) |
...
Admin 삭제·취소 및 플레이어 재제출 시 **변경 전 스냅샷**을 남깁니다. anon 접근 불가(service role만). Supabase 테이블에서 조회합니다.

| action | 발생 시점 | snapshot | 기타 |
|--------|-----------|----------|------|
| `delete_preference` | Admin Search → 요일별 Delete (배정 전) | 삭제된 preferences 행 | `actor_ip` |
| `cancel_reservation` | Schedule Grid → Cancel (배정 후) | 취소된 reservation 행 | `actor_ip` |
| `resubmit_preference` | 같은 사이클에 기존 preferences가 있는 상태에서 재제출 | 교체 전 preferences | `source`: `secret_url` \| `google_form`, `was_locked`: 배정 후 재제출 여부 |

> 감사 로그 기록 실패는 제출·삭제·취소를 **막지 않습니다** (콘솔에만 에러).
...
**공통:** 구글 폼·시크릿 URL 모두 `submitMultiDayReservationRpc` → Postgres RPC `submit_multi_day_reservation`을 호출합니다.  
(구글 폼: `app/api/google-form-submit/route.ts` → 시크릿 URL: `app/r/[token]/actions.ts`)  
스크립트용 래퍼 `processMultiDayReservation`도 동일 RPC로 위임합니다.

성공 시 한 트랜잭션 안에서 `players` upsert + 해당 `player_id`+`cycle_id`의 `preferences` DELETE 후 INSERT.  
재제출이면 RPC 호출 **전에** `audit_log`에 `resubmit_preference` 스냅샷을 남깁니다.

| 경로 | `reservation_open` 검사 | `last_assignment_run` 있을 때 |
|------|-------------------------|------------------------------|
| **구글 폼** | **검사 안 함** (`skipOpenCheck: true`) | RPC가 해당 player의 `reservations` DELETE 후 `preferences` 전체 교체 (assigned·eliminated 동일) |
| **시크릿 URL** | `actions.ts`에서 선검사 — `false`이면 **거부** (`SECRET_URL_CLOSED_MESSAGE`) | `reservation_open = true`이면 구글 폼과 동일 (`skipOpenCheck: true`로 RPC 호출) |
...
- 삭제 전 `audit_log`에 `delete_preference` 스냅샷 기록
...
- 취소 전 `audit_log`에 `cancel_reservation` 스냅샷 기록
...
| Open / Close secret URL | `reservation_open` 토글 — **시크릿 URL(`/r/...`)만** 통제 (구글 폼과 무관) |
| Export Excel | 사이클별 시트(요일별 등) |
| **Run full assignment** | `runFullBatchAssignment` — Search Reservations 위 노란 패널 |
| Reset cycle | `RESET` 입력 → `archive_and_reset_cycle` RPC로 아카이브 후 삭제, `current_cycle_id` +1 |
| Search | 배정 전: 신청자 검색 (요일별 Delete 버튼 포함) / 배정 후: 예약·대기 검색 |
| Delete application per day | 배정 전만 — 검색 결과에서 플레이어의 특정 요일 신청(`preferences`) 삭제 → `audit_log` |
| Applicants | 배정 전만 — `preferences` 기반 신청자 목록 |
| **Pending** | 배정 후만 — 마지막 배정 이후 재제출·신규로 `reservations`가 없는 신청자 (다음 Run full assignment에 포함) |
| Schedule Grid | 배정 후만 — UTC 그리드·슬롯별 Cancel → `audit_log` |
| Waitlist | 배정 후만 — 해당 요일 `eliminated` + 선호 블록 (`dedupeEliminatedByPlayer`로 player당 1행) |
| How to use | `/admin/guide` — Admin / Player / 스택 탭 |
...
- 배정 후 → `assigned` 슬롯 표시 + Waitlist(VP/MO) — eliminated는 player당 1행으로 중복 제거
- 마감 배너: `reservation_open === false` → *Secret URL applications closed*
...
| 계층 | 내용 |
|------|------|
| RLS | anon은 SELECT만 (`players`, `slots`, `reservations`, `preferences`, `reservation_open`) — `audit_log`는 service role만 |
| 쓰기 | Server Actions / API는 service role (`createServiceClient`); 신청은 `submit_multi_day_reservation` RPC |
| Admin | 세션 없으면 `requireAdmin()` 실패 |
| 토큰 URL | middleware + 서버에서 token 검증 |
| 웹훅 | `GOOGLE_FORM_WEBHOOK_SECRET` + timing-safe 비교 |
...
| 영역 | 파일 |
|------|------|
| 신청 RPC 래퍼·배정·MCMF | `lib/assignment.ts` (`submitMultiDayReservationRpc` / `processMultiDayReservation`) |
| 재제출 감사 로그 | `lib/audit-log.ts` |
| 재제출·메시지·eliminated 중복 제거 | `lib/reservation-guard.ts` |
...
| 스키마·RPC | `supabase/schema.sql` (`submit_multi_day_reservation`, `archive_and_reset_cycle`) |
| 마이그레이션 | `supabase/migrations/` (`v9_audit_log`, `v10_audit_log_resubmit` 등) |
...
두 경로 모두 `submit_multi_day_reservation` RPC를 거쳐 동일한 `players` / `preferences` 테이블에 씁니다. Apps Script는 Supabase를 직접 호출하지 않습니다. 재제출 시 API/Server Action이 RPC 전에 `audit_log`에 스냅샷을 남깁니다.

### 폼 상단 안내 문구 (복사용)

구글 폼 설명란에 아래 문구를 붙여 넣으세요. **이메일 수집을 켠 설정**(시트에 이메일 열이 생김) 기준이며, DB 반영은 **폼 재제출** 또는 **시크릿 링크**로 합니다(폼 응답 수정만으로는 웹훅이 다시 호출되지 않을 수 있음).
...
| 구글 폼 제출 후 수정 | **폼 재제출** 또는 시크릿 링크 재제출 (응답 수정만으로는 DB 미반영 가능) |
| 구글 폼 + 시크릿 링크, 동일 Player ID | **최신 제출이 이전 내용을 덮어씀** |
| 배정 실행 후 재제출 (`last_assignment_run`) | 기존 `reservations` DELETE + `preferences` 전체 교체 (구글 폼 항상 / 시크릿 URL은 `reservation_open = true`일 때) + `audit_log` |
| 시크릿 URL 마감 후 (`reservation_open = false`) | 시크릿 URL만 거부 — 구글 폼은 계속 접수 |

### 구글 폼 항목 구성

현재 Apps Script(`onFormSubmit.gs`)는 **이메일 수집 ON** 시트의 열 배치를 가정합니다.

| row 인덱스 | 항목명 | 유형 |
|-----------|--------|------|
| `row[0]` | 타임스탬프 | 자동 |
| `row[1]` | 이메일 주소 | 자동 (이메일 수집 ON) |
...
2. 폼 설정(톱니바퀴) → **응답** 탭:
   - 이메일 주소 수집: **켜기** — 시트 `row[1]`에 이메일이 들어가며, 현재 `onFormSubmit.gs` 인덱스와 맞춥니다 (이메일은 웹훅 payload에 **포함하지 않음**)
   - 응답 횟수 1회로 제한: **끄기** (한 사람이 **여러 Player ID**로 각각 제출해야 함)
   - 응답 수정 허용: 켜도 됨 — 다만 **응답 수정만으로는 DB가 갱신되지 않을 수 있음**. 변경은 **폼 재제출** 또는 시크릿 링크를 권장
3. 폼 완성 후 응답 탭 → 스프레드시트 아이콘 → **새 스프레드시트 만들기**
4. 테스트 제출 후 시트에서 `row[2]`가 Player ID인지 확인 (`onFormSubmit.gs` 인덱스와 일치해야 함)
...
DB 갱신은 **폼 재제출**(새 응답 → 웹훅) 또는 **시크릿 링크** 재제출로 처리합니다. 배정 실행 후에도 구글 폼 재제출은 허용되며, 기존 배정·대기 행이 삭제된 뒤 preferences가 교체됩니다.
...
| 신청 처리 함수 | `processReservation` (요일별 upsert) | `submit_multi_day_reservation` RPC (`submitMultiDayReservationRpc` / `processMultiDayReservation` 래퍼) |
| Reset 동작 | 데이터 소멸 | `archive_and_reset_cycle` RPC + `archived_*` 백업 후 삭제 |
| 신청 경로 | 시크릿 링크만 | 시크릿 링크 + 구글 폼 병행 |
| 시간 표시 | UTC/KST 토글 | UTC만 |
| Cancel 버튼 | 즉시 취소 | 로딩 스피너 + 완료 토스트 + `audit_log` |
| 배정 전 삭제 | 불가능 | 검색 결과에서 요일별 `preferences` 삭제 가능 (배정 전 한정, `audit_log`) |
| 배정 후 재신청 | — | Pending 패널에 표시, 다음 Run full assignment에 포함 |
| 구글 폼 시트 | (구) 이메일 없음 / Player ID=`row[1]` | 이메일 수집 ON — Player ID=`row[2]` |

---

*문서 기준: 저장소 `main` 브랜치 (MCMF 배정 + UTC 전용 UI + 구글 폼 파이프라인 + RPC 신청 + audit_log)*
