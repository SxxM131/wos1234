...
> English version: [RESERVATION_SYSTEM_EN.md](RESERVATION_SYSTEM_EN.md) · **모바일 HTML:** [RESERVATION_SYSTEM_EN.html](RESERVATION_SYSTEM_EN.html)
...
| C-2 | C 이후 | `/r/[token]` | 재신청 | — | 새 `preferences` (다음 배정 또는 수동 처리) |
...
| 4 | 취소된 플레이어 재신청 | 플레이어 `/r/[token]` | `clearCancelledDayReservations` 후 preferences 재생성 가능 |
