...
> **참고:** `players` 테이블에 `email` 컬럼(nullable)이 존재하나 현재 코드에서는 사용하지 않습니다. 중복 체크는 `game_id + cycle_id + day_of_week` 기준으로만 동작합니다.
...
`game_id + cycle_id + day_of_week` 기준으로 중복 여부를 확인합니다. 같은 요일에 이미 `preferences`가 존재하면 거부합니다. **다른 `game_id`로의 신청은 허용됩니다.**
...
4. 취소 완료 후 Admin UI에 토스트 알림 표시 ("취소 완료. 대기자가 있으면 자동 승격됩니다.").
...
| Schedule Grid | 배정 후만 — UTC 그리드·슬롯별 Cancel (취소 시 로딩 스피너 + 완료 토스트) |
...
|-----------|--------|------|------|
| `row[0]` | 타임스탬프 | 자동 | — |
| `row[1]` | 이메일 주소 | 자동 수집 | Apps Script에서 사용 안 함 (응답 수정 기능용) |
...
같은 `game_id`로 구글 폼과 시크릿 링크 양쪽에서 같은 요일을 신청하면 2번째 시도는 무시됩니다. **다른 `game_id`로의 신청은 두 경로 모두 허용됩니다.**
...
| 중복 체크 | email + game_id 혼용 | `game_id + cycle_id + day_of_week`만 |
