...
구글 폼은 **이메일 수집 없음** — 제출 후 폼으로 직접 수정할 수 없습니다.

1. **R4**에게 연락
2. R4가 Admin에서 해당 요일 신청을 삭제 (`Delete mon` / `Delete tue` / `Delete thu`)
3. R4가 제공한 **시크릿 URL**(`/r/[token]`)로 해당 요일을 다시 신청
...
|---|------|-----------|----------|-----------|
| A | 신청 기간 중 · **내용 수정 필요** | `/r/[token]` (R4 제공) | R4 Delete 후 시크릿 URL로 해당 요일 재신청 | Search → **Delete mon/tue/thu** |
...
| 2 | 같은 `player_id` + **같은 요일** + 사이클 재제출 | 거부 (요일당 1회) | *Contact r4 if you need changes.* |
| 3 | 마감 후 (`reservation_open = false`) | 거부 | *Reservations are closed* |
| 4 | 구글 폼 + 시크릿 URL **동일 Player ID·동일 요일** | 두 번째 채널 거부 | 한 채널만 사용; 폼 설명에 안내 권장 |
...
| 항목 | 내용 |
|------|------|
| 중복 방지 | `player_id + cycle_id + day_of_week`만 — **같은 Player ID·같은 요일은 1회** (구글 계정당 여러 Player ID 제출 가능, 응답 1회 제한 **끄기**) |
| 폼 안내 문구 | 폼 설명란에 중복 제출 안내 붙여넣기 — [§17 복사용 문구](docs/RESERVATION_SYSTEM.md#폼-상단-안내-문구-복사용) |
